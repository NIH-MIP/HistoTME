...
class WSI(Dataset):
    '''
    slide_dir: path to where all slide patches are stored
    
    '''
    def __init__(self, slide_dir, transforms = None) -> None:
        super().__init__()
        #HistoTME
        self.patches = glob.glob(os.path.join(slide_dir,'patches','*.png'))

        self.transforms = transforms

        try:
            #this is if patches were generated using CellVIT patch extraction scripts
            with open(os.path.join(slide_dir,'metadata.yaml'), 'r') as f:
                self.metadata = yaml.safe_load(f)
        except Exception as e:
            print('Error. no metadata file. Setting metadata to None...')
            self.metadata = None
    ...
    def __getitem__(self, index):
        ...
        # Apply transforms if any (e.g Macenko stain normalization, etc)
        img = cv2.cvtColor(cv2.imread(patch), cv2.COLOR_BGR2RGB)
        if self.transforms is not None:
            try:
                img = self.transforms(cv2.cvtColor(cv2.imread(patch), cv2.COLOR_BGR2RGB))
            except Exception as e:
        ...
        try:
            #other
            #edit this based on how the patches are named
            coords = np.array([int(os.path.basename(patch).split('_')[-2]), 
                               int(os.path.basename(patch).split('_')[-1].replace('.png',''))])
            

        except Exception as e:
            print(e)
            coords = np.array([int(patch.split('_')[-3].split('-')[0]), int(patch.split('_')[-3].split('-')[1])])
        return img, coords
    

class Macenko_Normalizer:
    '''
    Performing Macenko stain normalization on GPU using torch
